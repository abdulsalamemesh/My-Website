<script setup>
defineProps({classes:String});
</script>
<template>
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" :class="classes">
        <g fill="none" fill-rule="evenodd">
            <path fill="#9AA7B0" fill-opacity=".8" d="M3 3.1055C1.764 4.3685 1 6.0935 1 8.0005 1 9.9065 1.764 11.6315 3 12.8945L3 3.1055zM13 3.1055L13 12.8945C14.236 11.6315 15 9.9065 15 8.0005 15 6.0935 14.236 4.3675 13 3.1055"/>
            <path fill="#40B6E0" fill-opacity=".6" d="M12,2.2598 C10.865,1.4678 9.488,0.9998 8,0.9998 C6.512,0.9998 5.135,1.4678 4,2.2598 L4,13.7398 C5.135,14.5318 6.512,14.9998 8,14.9998 C9.488,14.9998 10.865,14.5318 12,13.7398 L12,2.2598 Z"/>
            <polygon fill="#231F20" fill-opacity=".7" points="5.5 1 5.5 0 .5 0 .5 1 2.5 1 2.5 6 3.5 6 3.5 1" transform="translate(5 5)"/>
        </g>
    </svg>
</template>


...
<template>
    <div class="h-screen font-montserrat flex flex-col md:flex-row">
        <div
            class="border-b-4 border-indigo-600 dark:border-white md:border-b-0 bg-indigo-50 dark:bg-black px-5 md:px-12 md:py-6 min-h-[4rem] flex md:flex-col justify-between items-center ">
            <button @click="toggleDark()"
                    class="rounded-md">
                <template v-if="isDark">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="h-6 w-6 text-white fill-current">
                        <path
                ...
                <template v-else>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="h-6 w-6 text-indigo-600 fill-current">
            ...
            <div>
                <button @click="showNav = !showNav" class="p-1 md:hidden">
                    <svg v-show="showNav === false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" class="h-6 w-6 text-black dark:text-white fill-current">
                        <path
                    ...
                    <svg v-show="showNav === true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" class="h-6 w-6 text-black dark:text-white fill-current">
                        <path
        ...
        >
            <div v-show="showNav" class="bg-black dark:bg-white md:hidden px-2 pb-2 pt-1">
                <div class="flex flex-col justify-center items-center space-y-2 bg-white dark:bg-black p-6 ">
                    <a href="#" class="font-bold text-black dark:text-white border-x-4 border-transparent hover:border-black dark:hover:border-white text-center py-2 px-6"
                         @click="showNav = false">
                        <span>-</span>
                        About me
                        <span>-</span>
        ...
        <div class="grow overflow-y-auto scroll-smooth scrollbar" @click="showNav = false">

            <div class="h-screen flex flex-col justify-center bg-indigo-50 dark:bg-black" id="nav">
                <div class="max-w-7xl mx-auto w-full grow flex justify-center items-center px-4 md:px-8">
                    <div class="flex flex-col h-full  justify-center w-full md:max-w-2xl md:space-y-3 ">

                          <p class="text-3xl md:text-5xl lg:text-6xl  dark:text-white font-semibold">Hi <span class="wave">&#128075;</span>, I'm</p>
                          <p class="text-3xl md:text-5xl lg:text-6xl  dark:text-white font-semibold">Abdulsalam Emesh</p>
                          <p class="text-xl md:text-2xl lg:text-3xl  dark:text-white font-semibold">I am a ...</p>
                          <VueWriter :array="title" :start="2000" :eraseSpeed="25" :typeSpeed="100" />

                    </div>
                </div>
            </div>
            <div class="h-screen flex flex-col justify-center bg-indigo-50 dark:bg-black" id="nav">
                <div class="max-w-7xl mx-auto w-full grow flex justify-center items-center">
                    <div class="flex flex-col justify-center items-center">
                        <p class="text-3xl text-red-500 dark:text-white font-semibold">I am a sa</p>
                        <VueWriter :array="title" :start="1000" :eraseSpeed="50" :typeSpeed="150"/>
                    </div>
                </div>
            </div>
        </div>
...
.is-typed {
    ...
    @apply text-4xl md:text-5xl lg:text-6xl font-bold text-indigo-500 dark:text-white;
}
...
@media only screen and (min-width: 451px) and (max-width: 1200px) {
    ...
    .scrollbar::-webkit-scrollbar-track {
        background-color: v-bind(isDark ? 'black': '#eef2ff');
    }

    .scrollbar::-webkit-scrollbar-thumb {
        background-color: v-bind(isDark ? 'white': '#a5b4fc' );
